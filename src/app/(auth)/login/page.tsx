import { redirect } from "next/navigation";
import { auth, signIn } from "@/auth";

export default async function LoginPage() {
  const session = await auth();
  if (session?.user) redirect("/dashboard");

  return (
    <div className="hero-bg min-h-screen flex items-center justify-center p-4">

      {/* Blobs */}
      <div aria-hidden className="pointer-events-none select-none">
        <div className="blob blob-1 w-[400px] h-[400px] top-[-80px] left-[-80px] opacity-25"
             style={{ background: "radial-gradient(circle, #3b82f6, #1d4ed8)" }} />
        <div className="blob blob-2 w-[350px] h-[350px] bottom-[-60px] right-[-60px] opacity-20"
             style={{ background: "radial-gradient(circle, #7c3aed, #4f46e5)" }} />
      </div>

      <div className="relative z-10 w-full max-w-sm">
        {/* Card */}
        <div className="glass rounded-3xl px-8 py-10 text-center"
             style={{ boxShadow: "0 32px 64px rgba(0,0,0,0.5)" }}>

          {/* Logo */}
          <div className="flex justify-center mb-6">
            <div className="w-14 h-14 rounded-2xl flex items-center justify-center"
                 style={{ background: "linear-gradient(135deg,#3b82f6,#7c3aed)", boxShadow: "0 0 32px rgba(59,130,246,0.4)" }}>
              <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 9.63 19.79 19.79 0 01.01 1A2 2 0 012 .84h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L6.09 8.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z"/>
              </svg>
            </div>
          </div>

          {/* Wordmark */}
          <h1 className="font-display font-800 text-2xl text-white tracking-tight mb-1">
            Meeting<span className="text-gradient">Alert</span>
          </h1>
          <p className="text-slate-400 text-sm mb-8 leading-relaxed">
            Sign in to connect your Google Calendar<br/>and start receiving call reminders.
          </p>

          {/* CTA */}
          <form action={async () => { "use server"; await signIn("google", { redirectTo: "/dashboard" }); }}>
            <button type="submit"
              className="w-full inline-flex items-center justify-center gap-3 font-display font-600 text-base py-3.5 px-6 rounded-xl text-white transition-all duration-200 hover:scale-[1.02]"
              style={{ background: "linear-gradient(135deg,#3b82f6,#7c3aed)", boxShadow: "0 0 28px rgba(59,130,246,0.3)" }}>
              <svg className="w-4 h-4" viewBox="0 0 24 24">
                <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="white"/>
                <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="white"/>
                <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="white"/>
                <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="white"/>
              </svg>
              Continue with Google
            </button>
          </form>

          <p className="text-slate-600 text-xs mt-6">
            Read-only calendar access · AES-256 encrypted
          </p>
        </div>
      </div>
    </div>
  );
}
